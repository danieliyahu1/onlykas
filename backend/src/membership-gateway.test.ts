...
describe("KaspaMembershipGateway", () => {
  ...
  it("builds SDK-readable genesis and mint transactions", async () => {
    const funding = (
      ...
      id: string,
      amount: string,
      script = addressScript(address).slice(4),
    ) => ({
        ...
        if (url.includes(encodeURIComponent(membershipAddress(minter))))
          return Response.json([
            funding(
              creator,
              "44".repeat(32),
              MEMBERSHIP_OUTPUT_VALUE.toString(),
              membershipScript(minter).slice(4),
            ),
            funding(
              creator,
              "33".repeat(32),
    ...
    expect(offerTransaction.outputs[0]?.covenant?.covenantId.toString()).toBe(
      offer.covenantId,
    );

    const mint = await gateway.prepareMint(
      creator,
      buyer,
      offer.covenantId,
      "1000000000",
    ...
    expect(mintTransaction.inputs[0]?.previousOutpoint.transactionId).toBe(
      "33".repeat(32),
    );
    expect(mintTransaction.inputs[0]?.signatureScript).toBeTruthy();
    expect(mintTransaction.outputs[1]?.covenant?.covenantId.toString()).toBe(
      offer.covenantId,
    );
    const inputValue = mintTransaction.inputs.reduce(
      (sum, input) => sum + (input.utxo?.amount ?? 0n),
      0n,
    );
    const outputValue = mintTransaction.outputs.reduce(
      (sum, output) => sum + output.value,
      0n,
    );
    expect(inputValue - outputValue).toBe(1_686_200n);
  });
  ...
  it("relays signed covenant transactions over wRPC", async () => {
    ...
    const submitTransaction = vi.fn(
      async (request: { transaction: unknown; allowOrphan?: boolean }) => {
        void request;
    ...
    await expect(
      submitMembershipTransactionOverWrpc(signed, () => ({
        connect,
        disconnect,
        submitTransaction,
      })),
