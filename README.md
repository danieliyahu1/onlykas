...
Development and production must use separate stateful resources. Local development uses the `kaskama-development` Turso database and the `kaskama-development` R2 bucket. Production uses the `kaskama-production` Turso database and the `kaskama-production` R2 bucket. Do not share database credentials or bucket-scoped R2 credentials between environments.
